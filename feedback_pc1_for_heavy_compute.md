...
**How to apply:**
- GA optimizer — уже на ПК1 через `/api/ga/run target=pc1` (см. project_ga_gpu_migration). **С 2026-05-10 PC1 — default везде**: HTML radio button checked=pc1, JS fallback `|| 'pc1'`, `index_v2.html` runGA шлёт `{target:'pc1'}`, `scripts/ga_weekly_trigger.py` тоже. Если хочешь VPS — нужно явно выбрать в radio. До 2026-05-10 был обратный дефолт и Артём кликнул кнопку → пошло на VPS на 31h.
