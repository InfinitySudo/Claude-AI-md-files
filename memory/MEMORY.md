...
- [Gerchik чек-лист расширен](project_gerchik_checklist_expanded.md) — дореализованы предпосылки из списков Артёма: +5 пробой (close-под-экстремум/откат<30%/консолидация/нет-реакции-ЛП/нет-импульса) +4 фейд (безоткатное/дистрибуция/импульс/задёрг); checklist_min 0.5→0.45 чтоб пробой не задохнулся
- [Gerchik гейты выбора стиля](project_gerchik_style_selection_gates.md) — 3 движка слепо параллельно (нет селектора); добавлены гейты режима курса: пробой←ближний ретест (44%), отбой/ЛП←дальний ретест (84/55%); atr_room для пробоя ВЫКЛ (зануляет); config style_gates*
- [Gerchik судья OAuth/429 + фейд-гейт](feedback_gerchik_judge_oauth_429_fade_gate.md) — 6 шортов TAO в SL (фейд +25% ракеты): fix FADE_MAX_COUNTER_ATR=1.0 (не фейдим ≥1×ATR против хода); API-ключ удалён ($5/0), OAuth 429 серверный (не интерактив Артёма), get_client api_key=None (env-перехват), use_vision=false→rule-судья
- [Gerchik отбой held-окно](feedback_gerchik_otboi_held_window.md) — отбой давал 0 сделок: held-фильтр «не пробит за всю историю 200D» зануляет отбой в крипте (триггер 0.13%); fix=окно последних N D (config otboi_held_lookback=60) → ×15 срабатываний; ЛП фирит 99%/пробой 5%/отбой 0.13%
- [Gerchik окно: двойная роль + лимитник](feedback_gerchik_window_dual_role_limit.md) — LEVEL_PROXIMITY в live = радиус рассмотрения сетапа (не только цена фила); 0.2% душил throughput (90% no_level_near); fix: окно→0.5% + entry_mode=limit (лимитник у уровня, on_price налив по ретесту, expiry 72ч) развязывает поток и качество фила
- [Gerchik отбор инструментов](project_gerchik_instrument_selection.md) — критерии Герчика (урок b95db6ef): ликвидность≥50M→энергия ATR≥1.5%→анти-памп→читаемость; добавил гарды свежесть/спайк/хвосты в is_tradeable; ⚠ 50M on-Bybit строг — режет мейджоры BNB/LINK/BCH (Герчик мерит TradingView-агрегат); рекоменд ~25-30M (scripts/gerchik_screen.py)
- [Gerchik ЛП ранний вход опровергнут](feedback_gerchik_lp_early_entry_refuted.md) — бэктест 24 монет: early-entry хуже ретеста; +КЛЮЧЕВОЕ: без модели слиппеджа расширение окна «помогало» (артефакт), СО слиппеджем (вход=market) чем УЖЕ тем лучше монотонно (0.3%:+546→0.8%:−339→1.5%:−1660R); откатил live 0.8%→0.5%; всегда моделировать слиппедж market-входа (scripts/bt_run.py)
...
- [PumpDump close-PnL vs Bybit + dust](feedback_pumpdump_real_qty_reconcile_dust.md) — close-PnL из ВНУТР. модели TP/BE занижал (TAO панель +$5.45 vs Bybit closedPnl +11.4948); сверка через /v5/position/closed-pnl (override .env, не test_key); правка записи + dust-fix (последний TP=весь остаток); TODO: тянуть closedPnl на закрытии
