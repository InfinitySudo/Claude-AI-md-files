...
Фикс: (1) **demote судит real только с `trader_promoted_at`** (или окно, что позже) → свежий promote имеет <demote_min_trades → hold; (2) **`promote_cooldown_hours`=24** — после demote paper-кандидат не переподнимается; (3) `apply()` пишет `trader_promoted_at`/`trader_demoted_at`. +6 тестов (301 passed). tradingbot перезапущен (код загружен). **Промоутер СНОВА ВКЛЮЧЁН** (`promo_enabled=true`, 2026-06-05 14:01) — флапа больше нет, dry-run: все paper `hold` (никто не проходит пороги WR55/PF1.3), F держится (6<15 сделок). ⚠ Если промоутер позже поднимет 2-го трейдера в real при живом F — они столкнутся на sub1 (One-Way), пока C/F не разведены по разным суб-аккаунтам (см. [[project-real-global-guard-limitation]]); max_concurrent_real=3.
