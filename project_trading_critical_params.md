...
SL_distance_USD = ATR × atr_multiplier
  где atr_multiplier = bot_settings.atr_multiplier = 0.15 (актуально на 2026-05-30; было 0.25 на 2026-05-26)
  ⚠ bot_settings — key-value таблица (setting_name/setting_value, 53 строки), НЕ wide-row и НЕ settings_json.
...
### AGGRESSIVE (REAL) — на 2026-05-30 ВЕЧЕР: TP1=0.6% фикс (percent-mode, БОЕВОЙ), atr_mult=0.15
- ⚡ **АКТУАЛЬНО: боевой AGGR TP1 = 0.6% от цены, 100% закрытие** (percent-mode через `bot_settings.tp1_percent=0.6`). config `tp_ratios[0]` (был 3.5R, потом Артём вручную 0.8R) теперь ИГНОРИРУЕТСЯ — percent-mode побеждает по свежести updated_at. Источник правды теперь bot_settings tp1..5_percent, НЕ config tp_ratios.
- (история) `tp_ratios: [3.5,...]` ← TP1=3.5R≈3.85% был недостижим (0 TP-хитов) — это драйвер всего анализа ниже.
- **TRAILING REDESIGN (план 2026-05-30, Артём)**: вместо фикс-SL — трейлинг на gap×R (R=ATR×0.15), TP1 40%@1.5R maker, 60% на трейле, SL-move=Market.
- ⚠️ **real_trades В PG ПОЛНЫЙ: 523 closed AGGR** (DB=trading_bot_v3, localhost). Прошлый промах — искал в несуществующей `trade_journal`. НО колонки **`peak_pnl_pct` == `trough_pnl_pct` во ВСЕХ 523** (финальный экстремум в сторону выхода, НЕ истинный MFE) → считать «как далеко в плюс» по ним НЕЛЬЗЯ, нужен kline-реплей.
- 🔑 **TRUE MFE через 1m-kline-реплей (`scripts/replay_pg_real_aggr.py` + `replay_trailing_bars.py`, кэш `data/klines/`, `backtest.fetch_klines` source='bybit' interval='1', ключ свечи 'startTime', DB Decimal→float перед json). 523 real AGGR since 2026-05-23, факт net −$86, avg notional $72.** Истинный MFE med=1.52% avg=1.79%, MAE med=1.05%. **Вход ЗДОРОВЫЙ: ≥+0.3% — 86%, ≥+0.5% — 78%, ≥+0.8% — 68%, ≥+1.0% — 63%, ≥+1.5% — 51%, ≥+2% — 35%.** Убыток ТОЛЬКО потому что TP1=3.5R≈3.85% недостижим → победители откатываются в BE/SL.
- 🔑 **Per-bar honest реплей (консервативный порядок бара):**
  - ⚠️ **Фикс-% БЬЁТ фикс-R (важно, против первой рекомендации!):** фикс-TP в % — 0.6%→+$41 (best), 0.8%→+$21. Фикс-TP в R — **0.5R→+$20, 0.6R→+$0.5, 0.7R→−$11, 1.0R→−$32** (только 0.5R плюс, дальше минус). Причина: R=ATR×0.15 разброс p10=0.47% p90=1.95%; R-TP на шир-ATR монете = далеко (1.2%+, мало хитов), на узкой = слишком близко. Фикс-% 0.6% попадает в sweet-spot лучше. **РЕКОМЕНДАЦИЯ ПЕРЕСМОТРЕНА: боевой TP1 = 0.6% от цены (фикс), НЕ в R.**
  - **Трейлинг Артёма (TP1 40%@1.5R + 60% trail, hard SL=1R) ОТРИЦАТЕЛЬНЫЙ: act1.0/gap1.0 → −$48; act1.5/gap1.0 → −$57; act2.0/gap1.5 → −$72.** Лучше факта (−86), но НАМНОГО хуже фикс-TP.
  - **ПОЧЕМУ трейл проигрывает**: ждёт активации (1.0-2.0R) при hard SL=1R всё это время → 164 BE-сделки (сейчас ≈breakeven) уходят в полный −1R. Ранний фикс-TP забирает +0.6% у 74% сделок — это эдж.
- **ВЫВОД (data-backed, honest per-bar): главный рычаг = достижимый фикс-TP ~0.6% вместо 3.5R (−$86 → +$41). Трейлинг ≥1.5R СТАТИСТИЧЕСКИ ХУЖЕ — убивает BE-защиту. Если внедрять трейл — только shadow поверх фикс-TP1, не как замену SL.** Caveat: fill TP оптимистичен (касание лимита), 8 дней/523 сделки.
- ✅ **SL-GRID С BE-МОДЕЛЬЮ — ВАЛИДНЫЙ ОТВЕТ (2026-05-31, `scripts/replay_sl_grid_be.py`):** прошлый grid (`replay_sl_tp_grid.py`) был НЕВАЛИДЕН (confound: брал затёртый BE sl_price). Чистый: берёт `initial_sl_price` (исходный ATR-стоп) + РЕ-симулирует BE-move (1.3%→entry+0.6%) одинаково для всех ячеек. Sanity: модель baseline 0.15/0.6 = $4.6 ≈ факт $2.78 на тех же 51 сделках ✓ (BE-модель верна). ⚠️ Полная usable-выборка (391) смещена (исключает BE-победителей без initial_sl) → абсолютные суммы там мусор; чистые только 51 строка с initial_sl.
- 🔑 **РЕЗУЛЬТАТ на 42 чистой (фикс-TP era, mix TP1=18/BE=13/SL=11):** sanity модель baseline 0.15/0.6=$6.1 vs факт $0.5 (модель оптимистична на ~$5.6 из-за touch=fill TP, но направление верно). **СУЖЕНИЕ SL ДЕЛАЕТ ХУЖЕ по net:** 0.08→−$2..−5, 0.10→−$0.4..−2 (минус!), 0.12→+$1..+3, **0.15 (текущий)→+$6..+8.5 (топ)**, 0.20→+$5..+8. Узкий стоп выбивается шумом до TP. **R:R растёт при сужении (0.08→0.7-1.0 vs 0.15→0.4-0.8) но net падает — классический trade-off, и net тут главнее.** По TP: 0.8-1.0% слегка бьёт 0.6% (0.15/0.8=$8.5 vs 0.15/0.6=$6.1) — БОЛЬШЕ сделок дотягивает в боковике чем кажется. **ВЫВОД: atr_mult НЕ снижать (Артёмова интуиция про R:R не работает — net хуже); текущий 0.15 оптимален; возможный твик — TP1 0.6→0.7-0.8% (но n=42, разница в пределах шума).** Главный рычаг остаётся: фильтр входа (chop) + shadow-трейл.
- 🐞 **shadow-хук молчал — фикс `_read_setting` NameError (commit 82308ce 2026-05-31):** `env_config` не импортирован на module-scope → bare `env_config.get_db_params()` кидал NameError → except → return default → `trailing_shadow_enabled` всегда читался False, хук тихо no-op на каждом закрытии (0 строк за часы). Фикс: `import env_config as _ec` локально + лог исключения. Теперь читает True.
- ✅ **ВНЕДРЕНО 2026-05-30 вечер (план `cheeky-wondering-shell`, master HEAD a41b78e, не запушено):**
  - **Часть A (БОЕВАЯ):** `bot_settings.tp1_percent=0.6` (+tp2..5 заглушки) → `risk_manager_v3._resolve_tp_source` в **percent-mode** (TP1=0.6% от цены, 100% закрытие, побеждает config tp_ratios по свежести updated_at). SL=ATR×0.15 не тронут. +5 ключей `tp{1..5}_percent` в SETTINGS_REGISTRY (backend db). Подтверждено: resolver MODE=percent, TP1=entry×1.006. **Откат:** удалить tp*_percent.
  - **Часть B (shadow, zero-risk):** `src/trailing_sim.py` (`simulate_trailing`) + хук `_maybe_write_trailing_shadow` inline в `order_executor_wrapper_v3._reconcile_real_trades_once` (после update status=closed). На закрытии real AGGR реплеит 1m-свечи → shadow-строка через `database_v3.insert_real_shadow_trade` (класс БД = `DatabaseConnector`!). Колонка `real_trades.initial_sl_price` (migrate applied) хранит исходный 1R. Флаги: `trailing_shadow_enabled=true`, act=1.5R gap=1.0R tp1=1.5R frac=0.40. **Откат:** `trailing_shadow_enabled=false` + DELETE shadow-строк.
  - **pytest: 255 passed, 0 failed.** ⚠️ Грабли: git-stash для baseline-проверки уронил правки database_v3 (пришлось переписать); dashboard-api надо рестартить чтобы registry подхватил новые ключи (1-й POST дал "unknown key").
  - **Мониторинг через ~1-2 нед:** SUM realized боевой (is_shadow=false) vs shadow (TRAIL_SHADOW) по AGGR. Ожидание: фикс-TP бьёт трейл.
- (история) `tp_ratios: [2.5,...]` было на 2026-05-26 (TP1=2.5R по прогнозу +$35/мес)
